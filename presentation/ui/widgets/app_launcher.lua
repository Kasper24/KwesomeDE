...
local function create_app_widget(self, entry)
    ...
    function app.spawn()
        if entry.terminal ~= true then
            Gio.AppInfo.launch_uris_async(Gio.AppInfo.create_from_commandline(entry.executable, nil, 0))
        else
            helpers.run.exec_terminal_app(entry.executable)
        end
...
local function generate_apps(self)
    ...
    if self.sort_alphabetically then
        table.sort(apps, function(a, b)
            ...
            if helpers.table.has_value(self.favorites, app_info.get_name(a)) then
                app_a_score = "aaaaaaaaaaa" .. app_a_score
            end
            local app_b_score = app_info.get_name(b):lower()
            if helpers.table.has_value(self.favorites, app_info.get_name(b)) then
    ...
    elseif self.reverse_sort_alphabetically then
        ...
        table.sort(apps, function(a, b)
            local app_a_favorite = helpers.table.has_value(self.favorites, app_info.get_name(a))
            local app_b_favorite = helpers.table.has_value(self.favorites, app_info.get_name(b))
    ...
    for _, app in ipairs(apps) do
        if app.should_show(app) then
            ...
            local icon = helpers.icon_theme:get_gicon_path(app_info.get_icon(app))

            -- Check if this app should be skipped, depanding on the skip_names / skip_commands table
            if not helpers.table.has_value(self.skip_names, name) and not helpers.table.has_value(self.skip_commands, commandline) then
                ...
                if icon ~= "" or self.skip_empty_icons == false then
                    if icon == "" then
                        if self.default_app_icon_name ~= nil then
                            icon = helpers.icon_theme:get_icon_path(self.default_app_icon_name)
                        elseif self.default_app_icon_path ~= nil then
                            icon = self.default_app_icon_path
                        else
                            icon = helpers.icon_theme:choose_icon({"application-all", "application", "application-default-icon", "app"})
                        end
                    end
