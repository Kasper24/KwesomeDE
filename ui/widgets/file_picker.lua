...
function file_picker:set_initial_value(initial_value)
    self._private.initial_value = initial_value
    self._private.text_input:set_initial(initial_value)
end

function file_picker:set_on_changed(on_changed)
...
local function new()
    ...
    local set_folder_button = wibox.widget {
        ...
        text = "...",
        on_release = function()
            awful.spawn.easy_async(FILE_PICKER_SCRIPT_PATH .. " '" .. widget._private.initial_value .. "'", function(stdout)
...
function file_picker.mt:__call()
    return new()
end

return setmetatable(file_picker, file_picker.mt)