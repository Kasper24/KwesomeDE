...
local function icons()
	theme.icons = {
		...
		thermometer = {
			quarter = { icon = "", size = 30 },
			half = { icon = "", size = 30 },
			three_quarter = { icon = "", size = 30 },
			full = { icon = "", size = 30 },
		},
		network = {
			wifi_off = { icon = "󰤭" },
		...
		code_pull_request = { icon = "" },
		commit = { icon = "" },
		star = { icon = "" },
		code_branch = { icon = "" },

		gamepad_alt = { icon = "" },
		lights_holiday = { icon = "󱊺" },
		download = { icon = "" },
		speaker = { icon = "󰓃" },
		archive = { icon = "" },
		unlock = { icon = "" },
	...
	local function set_icon_default_props(icon, color)
		...
		if icon.font == nil then
			icon.font = "Iosevka Nerd Font Mono "
		end
	...
	function theme.get_svg_icon(names)
		...
		if icon_info then
			...
			if not theme.svg_icons[icon_path] then
