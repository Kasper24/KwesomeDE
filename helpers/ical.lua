...
function parser.VEVENT(entry, k, v)
    if k == "BEGIN" then
        function entry:duration(f)
            return ical.duration(self, f)
        end
        function entry:is_in(s)
            return ical.is_in(self, s)
        end
        function entry:is_over(s)
            return ical.is_over(self, s)
        end
...
function parser.STANDARD(entry, k, v)
    if k:find("DTSTART") or k:find("RRULE") then
        ...
        if err and err ~= "RRULE.UNTIL or RRULE.COUNT not found" then
            return err
        end
...
local function apply_rrule(orig, tb)
    if not orig or type(tb) ~= "table" then
...
function ical.events(cal)
    if type(cal) ~= "table" or cal.type ~= "VCALENDAR" then
        return nil
    end
    local evs = {}
...
function ical.is_in(e, s)
    if type(e) ~= "table" or type(s) ~= "table" then
        return nil
    end
