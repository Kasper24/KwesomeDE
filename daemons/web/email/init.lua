...
function email:get_address()
    if self._private.address == nil then
        self._private.address = Secret.password_lookup_sync(self._private.address_schema, self._private.address_atrributes)
    end
...
function email:get_app_password()
    if self._private.app_password == nil then
        self._private.app_password = Secret.password_lookup_sync(self._private.app_password_schema, self._private.app_password_atrributes)
    end
...
function email:refresh()
    ...
    file:read(function(error, content)
        ...
        awful.spawn.easy_async(string.format("curl -f %s -u %s:'%s'",
            self:get_feed_address(),
            self:get_address(),
